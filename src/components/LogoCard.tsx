 ...
 * - Triggers parent download modal for PNG export
 * - Animated entrance/exit via framer-motion
 */
import { useState, useRef, forwardRef, useCallback } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { Copy, Pause, Play, Check, Download } from 'lucide-react';
...
export const LogoCard = forwardRef<HTMLDivElement, LogoCardProps>(
  ({ logo, onDownloadPNG }, ref) => {
    ...
    const handleDownloadClick = useCallback(
      (e: React.MouseEvent) => {
        ...
        try {
          ...
          onDownloadPNG?.(svgHTML, logo.motif, logo.id);
        } finally {
          setTimeout(() => setDownloading(false), 500);
        }
      },
      [logo.motif, logo.id, onDownloadPNG]
    );
          ...
          {isHovered && (
            ...
            >
              ...
              <button
                ...
                className="flex flex-col items-center justify-center p-3 rounded-full hover:bg-[#2f2723] transition-colors text-[#e8e3df] disabled:opacity-50"
                title="Download PNG"
